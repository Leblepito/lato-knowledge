#!/usr/bin/env python3
"""
⚠️ DEPRECATED (2026-07-23) — LINE köprüsü devre dışı bırakıldı.

Karar: Her şey Telegram içinde çalışacak — personel input dosyalarını doğrudan
departman topic'lerine atıyor. Yeni sistem: telegram-bot/lato_telegram_bot.py
(Claude Sonnet 5, abonelik üzerinden ücretsiz). Bu dosya arşiv amaçlı tutuluyor;
deploy ETMEYİN.
...
# Sınıflandırma: Claude Sonnet 5 (LATO_AI_MODEL env ile değiştirilebilir)
OR_MODEL = os.environ.get("LATO_AI_MODEL", "anthropic/claude-sonnet-5")

TG_GROUP = -1003776134843
# AGENTS.md topic şemasıyla hizalı: 130 Elektrik&Havuz, 131 Teknik Bakım, 135 IT&Muhasebe
TG_TOPICS = {
    "ON_BURO": 132, "HK": 132, "FB": 134,
    "ELEKTRIK": 130, "TEKNIK": 131,
    "GUVENLIK": 132, "MUHASEBE": 133, "IT": 135, "YONETIM": 1,
}

# Webhook'tan spawn edilen task'ler GC'ye gitmesin
_BG_TASKS: set = set()
...
FLAG = {"tr": "🇹🇷", "th": "🇹🇭", "en": "🇬🇧"}
DEPT_FLAG = {
    "ON_BURO": "🛎️", "HK": "🧹", "FB": "🍽️", "ELEKTRIK": "⚡", "TEKNIK": "🔧",
    "GUVENLIK": "🔒", "MUHASEBE": "💰", "IT": "💻", "YONETIM": "🏨",
}


def parse_ai_json(content: str) -> dict:
    """Model çıktısından JSON çıkar — Claude'un ```json çitlerini de tolere eder."""
...
async def line_reply(reply_token: str, messages: list[dict]):
    ...
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(f"{LINE_MSG_API}/message/reply", json={
            "replyToken": reply_token,
            "messages": messages,
        }, headers={"Authorization": f"Bearer {LINE_TOKEN}",
                     "Content-Type": "application/json"})
        if r.status_code != 200:
            logger.error(f"LINE reply {r.status_code}: {r.text[:150]}")

async def line_push(to_user_id: str, messages: list[dict]):
    """LINE'a push mesaj (proaktif) — free planda kapalı olabilir."""
    if not to_user_id:
        return
    async with httpx.AsyncClient(timeout=30) as c:
        r = await c.post(f"{LINE_MSG_API}/message/push", json={
            "to": to_user_id,
            "messages": messages,
        }, headers={"Authorization": f"Bearer {LINE_TOKEN}",
                     "Content-Type": "application/json"})
        if r.status_code != 200:
            logger.error(f"LINE push {r.status_code}: {r.text[:150]}")
...
async def tg_send(text: str, topic_id: int = 132):
    """Markdown (tek yıldız bold) gönder; parse hatasında düz metne düş."""
    async with httpx.AsyncClient(timeout=30) as c:
        ...
        if r.status_code != 200:
            logger.error(f"TG {r.status_code}: {r.text[:150]}")
...

{
  ...
  "departman": "ON_BURO | HK | FB | ELEKTRIK | TEKNIK | GUVENLIK | MUHASEBE | IT | YONETIM",
...
- Elektrik/pano/havuz/pompa/jeneratör → ELEKTRIK
- Klima/AC/tesisat/su kaçağı/onarım/bina bakım → TEKNIK
- WiFi/internet/ağ/PMS/bilgisayar → IT
- Oda temizlik → HK
- Misafir/check-in/rezervasyon → ON_BURO
- SADECE JSON dön, markdown çiti (```) veya başka metin yazma"""
...
async def classify_message(text: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as c:
            ...
            if r.status_code != 200:
                raise RuntimeError(f"OpenRouter {r.status_code}: {r.text[:150]}")
            data = r.json()
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as c:
            r = await c.post(full_url, json={
                "contents": [{
                    "parts": [
                        {"text": "Bu bir fatura/dekont fotoğrafı. SADECE şu anahtarlarla JSON dön: "
                                 '{"firma": "şirket adı", "tutar": 0.0, "para_birimi": "THB", '
                                 '"tarih": "YYYY-MM-DD", "fatura_no": ""}. '
                                 "Okunamayan alanı null yap. Markdown çiti kullanma."},
...
async def process_text_message(text: str, staff: dict, reply_token: str):
    ...
    tg_lines = [
        ...
        f"👤 *{name}* ({dept}) | {FLAG.get(str(src_lang).lower(), '')} {src_lang}",
        f"📦 {msg_type} | {priority} | {DEPT_FLAG.get(target_dept, '')} {target_dept}",
        "",
        f"💬 {text}",
        "",
    ]
...
async def process_image_message(message_id: str, staff: dict, reply_token: str):
    ...
    # Telegram'a gönder
    try:
        ocr_data = parse_ai_json(ocr_result) if ocr_result else {}
        try:
            tutar = float(ocr_data.get("tutar") or 0)
        except (TypeError, ValueError):
            tutar = 0.0
        firma = ocr_data.get("firma") or "?"
        birim = ocr_data.get("para_birimi") or "THB"
        caption = (
            f"💵 FATURA/GİDER (LINE)\n"
            f"👤 {name}\n"
            f"🏢 {firma}\n"
            f"💰 {tutar:,.0f} {birim}\n"
            f"📅 {ocr_data.get('tarih') or '?'} | 🧾 {ocr_data.get('fatura_no') or '—'}"
        )
        ...
        logger.error(f"OCR işleme: {e}")
        await tg_send_photo(tmp.name, f"📷 LINE foto ({name}) — OCR başarısız, manuel kontrol", TG_TOPICS["MUHASEBE"])
...
async def handle_register(text: str, staff: dict, reply_token: str):
    """/register İsim Departman — personel kaydını registry'ye yazar."""
    valid_depts = sorted(set(TG_TOPICS) | {"GENEL"})
    parts = text.split(maxsplit=2)
    if len(parts) < 3:
        await line_reply(reply_token, [{
            "type": "text",
            "text": "📝 Kayıt için:\n/register [İsim] [Departman]\n\n"
                    "Örnek:\n/register Somchai ELEKTRIK\n/register Nong HK\n/register Kanya FB\n\n"
                    f"Departmanlar: {', '.join(valid_depts)}"
        }])
        return
    ...
    if dept not in valid_depts:
        await line_reply(reply_token, [{
            "type": "text",
            "text": f"⚠️ Bilinmeyen departman: {dept}\nGeçerli: {', '.join(valid_depts)}"
        }])
        return

    line_user_id = staff.get("line_user_id", "")
    if not line_user_id:
        logger.warning("register: line_user_id yok, kayıt atlanıyor")
        return

    registry = load_staff()
    registry["users"][line_user_id] = {
        "name": name,
        "department": dept,
        "role": "PERSONEL",
    ...
    save_staff(registry)
    logger.info(f"✅ Kayıt: {name} → {dept} ({line_user_id[:8]}...)")

    await line_reply(reply_token, [{
        "type": "text",
        "text": f"✅ Kayıt tamamlandı!\n👤 {name}\n🏢 {DEPT_FLAG.get(dept, '')} {dept}\n\n"
                f"Artık mesajların otomatik olarak doğru ekibe yönlendirilecek."
    }])
    await tg_send(f"👋 *Yeni personel kaydı* (LINE)\n👤 {name} → {DEPT_FLAG.get(dept, '')} {dept}",
                  TG_TOPICS["YONETIM"])
...
async def line_webhook(request):
    ...
    for event in data.get("events", []):
        ...
        if msg_type == "text":
            _spawn(process_text_message(msg["text"], staff, reply_token))
        elif msg_type == "image":
            _spawn(process_image_message(msg["id"], staff, reply_token))
        elif msg_type == "audio":
            _spawn(process_audio_message(msg["id"], staff, reply_token))
...
def verify_signature(body: str, signature: str) -> bool:
    ...
    if not LINE_SECRET:
        # Güvenlik: secret yoksa isteği REDDET (sadece açıkça izin verilirse kabul et)
        if os.environ.get("LINE_ALLOW_INSECURE") == "1":
            logger.warning("⚠️ LINE_ALLOW_INSECURE=1 — signature doğrulaması atlanıyor (sadece dev)")
            return True
        logger.error("LINE_CHANNEL_SECRET tanımsız — istek reddedildi")
        return False
    hash_val = hmac.new(
